<?php
    ...
    if (isset($_GET['pNo'])) {
        $program =
            ProgramController::getProgram($_GET['pNo']);
    ...
    if (isset($_POST['save'])) {
        ...
        $program = new Program($_POST['name'],$_POST['address'], $_POST['treatment'], $_POST['beds']);
    ...
    //Validation beds namespace numeric check
    try {
        Validation\numericValue($beds);
    }
    catch (Exception $e) {
...
<body>
    ...
    <h3><?php
        if (strlen($name_error) > 0 || strlen($address_error) > 0 || strlen($treatements_error) > 0 || strlen($beds_error) > 0) { 
