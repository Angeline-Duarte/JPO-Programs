<?php
    ...
    $userId_error = Validation\stringValid($userId, 10);  
    $password_error = Validation\stringValid($password, 5);  
    $fName_error = Validation\stringValid($fName, 15);  
    $lName_error = Validation\stringValid($lName, 15);  
    $hireDate_error = Validation\stringValid($hireDate, 15);   
    ...
    try {
        Validation\numericValue($userLevelNo);
    }
    catch (Exception $e) {
        $userLevelNo_error = $e->getMessage();
    }
?>
<html>
<head>
...
<body>
    ...
    <form method='POST'>
        <h3>User ID: <input type="text" name="userId" required
